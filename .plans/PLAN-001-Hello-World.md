...
> updated_at: 2026-04-16 12:39:00
...
- **C-002**: 目标版本组合（暂定）：
  ...
  - Android Studio 待确定
  - Android 真机系统版本 待确定
- **C-003**: 仅使用 Flutter 官方内置 Widget 与 Material 库，不引入第三方 package
- **C-004**: 每个 Dart 文件不超过 100 行，便于作为参考示例阅读

### Assumptions

- **A-001**: 使用 Flutter 3.29.3；`flutter create` 默认模板使用 Material 3
- **A-002**: 项目根目录下允许创建 `hello_world/` 子目录存放 Flutter 工程
...
- [ ] **SPEC-001**: Flutter 环境版本验证（→ G-001）
  ...
  - **验收（勾选即证据）**:
    ...
    - [ ] Flutter 版本为 `3.29.3`
    - [ ] Java 版本为 `17`
    - [ ] AGP / Gradle / Kotlin 版本在 `flutter build apk` 产出的 Gradle 构建日志中可确认

- [ ] **SPEC-002**: Android Studio Debug 配置验证（→ G-002）
...
- [ ] **SPEC-003**: Flutter 工程初始化与 APK 构建（→ G-003）
  ...
  - **关键决策**: 使用 `flutter create --org com.lisitede.preset.flutter hello_world`；`lib/main.dart` 仅保留 `MaterialApp` + 一个 Stateless Widget 展示欢迎文字，不引入第三方依赖
  - **实现约束**:
    - 不添加第三方 package
    - `lib/main.dart` 不超过 40 行
  - **接口 / 对接点**: N/A
  - **命令 / 操作**:
    - `flutter create --org com.lisitede.preset.flutter hello_world`
    - `flutter build apk`
    - `flutter run`
  - **验收（勾选即证据）**:
...
- [ ] **TASK-001**: 在仓库根目录执行 `flutter create --org com.example hello_world`，确认工程文件生成无误
  - **Complexity**: Low
